...
The auto-hwp document engine compiled to WebAssembly — **open `.hwp`/`.hwpx` in the browser, render
...
L3 @auto-hwp/react        — optional React binding (useHwpEditor + components); all replaceable
L2 @auto-hwp/editor-core  — headless editor (DocSession/SelectionModel/EditController) over an EngineAdapter
L2' @auto-hwp/ai-protocol — vendor-neutral LLM protocol (prompt/context/validate); no fetch, no keys
L1 @auto-hwp/engine (this) / hwp-mcp — headless engine; state is the document only
```

A `WasmAdapter` (in `@auto-hwp/react`) wraps this package to satisfy L2's `EngineAdapter` contract, so the
...
engine errors (`no_document`, `font_missing`, …) pass through unchanged. `@auto-hwp/react`'s
