/// SVG sanitizer (R7) — copied from crates/hwp-viewer/ui/src/sanitize.ts, the verified desktop-app
/// defense, and made the SINGLE gate every SVG string passes through before it is injected via
/// dangerouslySetInnerHTML (HwpPageView). A malicious .hwp/.hwpx can embed <script>, on* handlers,
/// javascript: URIs, or <foreignObject> (arbitrary HTML) in its rendered SVG — all of which would run
/// in the host page. We parse the markup as SVG, walk the tree, and strip the dangerous nodes/attrs,
/// then re-serialize. Structural stripping (not regex) is robust against attribute-injection. CSP is
/// the second layer; this is defense-in-depth.
///
/// R7 CONTRACT: the package exposes NO prop or API that takes an SVG string and injects it directly.
/// HwpPageView pulls the SVG from the adapter and ALWAYS runs it through `sanitizeSvg` here first, so
/// no dangerouslySetInnerHTML value ever escapes this gate.
...
/** Return a sanitized copy of an SVG string safe to inject via innerHTML. Refuses (returns "") on
 *  parse error or a non-<svg> root rather than injecting anything questionable. */
