...
export type SiteNavKey = "home" | "bulk" | "bench" | "docs" | "models" | "download";
...
// 데스크톱 앱은 **내비가 아니라 버튼**으로 둔다. 다른 항목은 "읽으러 가는 곳"이고 이것만
// "가져가는 것"이라, 같은 줄에 같은 무게로 두면 무엇을 하라는 페이지인지 흐려진다.
const DOWNLOAD_NAV = {
  href: siteHref("/download"),
  label: "앱 내려받기",
  title: "macOS 데스크톱 앱 — Apple 공증됨, 문서는 기기를 떠나지 않습니다",
};
  ...
  return (
    <header className={styles.header}>
      ...
      <a
        ...
        title={DOWNLOAD_NAV.title}
        aria-current={current === "download" ? "page" : undefined}
      >
        <Download size={13} aria-hidden /> {DOWNLOAD_NAV.label}
      </a>
