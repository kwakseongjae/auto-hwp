...
test("⌘/Ctrl+휠 줌: 커서 아래 문서 지점이 고정된다 (커서 중심 줌, ±2px)", async ({ page }) => {
  ...
  // 커서 P = 시트 ∩ 캔버스(가시 뷰포트)의 겹침 영역 안의 한 지점. 시트가 캔버스보다 넓거나(가로 스크롤)
  // 좌측 아웃라인 패널(issue 046)로 캔버스가 좁아진 경우, 시트의 기하학적 중심은 캔버스 밖(채팅 패널 위)
  // 으로 벗어날 수 있다 — 그러면 합성 휠이 .hw-canvas 리스너에 도달하지 못한다. 겹침 영역으로 클램프해
  // P 가 항상 캔버스 안의 시트 위에 오도록 한다(fracX/fracY 는 시트 기준이므로 고정점 검증 불변).
  const cb = await page.locator(".hw-canvas").boundingBox();
  if (!cb) throw new Error("캔버스 박스를 찾지 못함");
  const ovLeft = Math.max(b0.x, cb.x);
  const ovRight = Math.min(b0.x + b0.width, cb.x + cb.width);
  const ovTop = Math.max(b0.y, cb.y);
  const ovBot = Math.min(b0.y + b0.height, cb.y + cb.height);
  const P = { x: (ovLeft + ovRight) / 2, y: ovTop + (ovBot - ovTop) * 0.4 };
