...
impl OllamaProvider {
    ...
    fn chat(&self, system: &str, user: &str) -> Result<String> {
        ...
        let resp = reqwest::blocking::Client::new()
            ...
            .send()
            .map_err(|e| {
                Error::Other(format!("ollama request failed (is `ollama serve` running?): {e}"))
            })?;
        let status = resp.status();
        let val: serde_json::Value =
            resp.json().map_err(|e| Error::Other(format!("ollama response decode failed: {e}")))?;
        if !status.is_success() {
            let msg = val.pointer("/error").and_then(|m| m.as_str()).unwrap_or("unknown error");
            return Err(Error::Other(format!("ollama API {status}: {msg}")));
        }
        Ok(val.pointer("/message/content").and_then(|c| c.as_str()).unwrap_or_default().to_string())
    }
}
