...
fn hwpx_origin_cell_edit_exports_table_at_original_anchor() {
    ...
    let ti = *tables_before.first().expect("fixture has a top-level table");
    ...
    let out = apply_intent(
        &mut session,
        Intent::SetTableCell { section: 0, index: ti, row: 0, col: 0, text: MARKER.into() },
    )
    ...
    assert_eq!(table_indices(&after).len(), tables_before.len(), "no duplicate table at the end");
    assert_eq!(after.sections[0].blocks.len(), n_blocks, "block count unchanged");
    ...
    let cell_text: String = t
        ...
        .unwrap_or_default();
    assert!(cell_text.contains(MARKER), "edited cell text sits at the ORIGINAL anchor: {cell_text:?}");

    // ③ 마커는 문서 전체에서 정확히 한 번.
    assert_eq!(after.plain_text().matches(MARKER).count(), 1, "edited text appears exactly once");

    // ④ 재수출 결정성 + 오픈 세이프티.
    assert_eq!(export_bytes(&session).unwrap(), bytes, "export is byte-deterministic");
    assert!(hwp_core::validate_hwpx(&bytes).ok, "exported package is open-safe");
}
