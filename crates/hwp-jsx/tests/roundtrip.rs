...
fn t1_roundtrip_all_fixtures() {
    for name in FIXTURES {
        ...
        assert!(
            ...
            "[{name}] doc_value_eq FAILED (content lost in codec)"
        );
        ...
        assert_eq!(
            fp1, fp2,
            "[{name}] fixed-point FAILED: emit(parse(emit))) != emit"
        );
...
fn t1b_equality_is_falsifiable() {
    ...
    if let Some(Block::Paragraph(p)) = back.sections.get_mut(0).and_then(|s| s.blocks.get_mut(0)) {
        ...
        back.char_shapes.push(CharShape {
            bold: true,
            ..Default::default()
        });
    }
    assert!(
        !doc_value_eq(&doc, &back),
        "equality must FAIL on a mutated doc"
    );
}
...
fn t1c_per_edge_borders_and_diagonal_roundtrip() {
    ...
    let black = Color {
        ...
        b: 0,
        a: 255,
    }; // opaque black (codec colors are always opaque)
    let cell = Cell {
        row: 0,
        col: 0,
        ...
        diagonal: Some(CellDiagonal {
            kind: DiagonalKind::Slash,
            color: red,
            width_px: 1.0,
        }),
        ..Default::default()
    ...
    if let Some(Block::Table(t)) = tampered.sections[0].blocks.get_mut(0) {
        t.cells[0].borders[0] = Some(CellEdge {
            color: blue,
            style: LineStyle::Solid,
            width_px: 2.0,
        });
    ...
    assert!(
        !doc_value_eq(&doc, &tampered),
        "equality must catch a changed edge style"
    );
}
...
fn t2_css_only_op_leaves_jsx_byte_identical() {
    ...
    let jsx_before: Vec<String> = std::iter::once(&proj.document)
        .chain(&proj.sections)
        .map(hwp_jsx::jsx::emit_jsx)
        .collect();
    ...
    let jsx_after: Vec<String> = std::iter::once(&proj.document)
        .chain(&proj.sections)
        .map(hwp_jsx::jsx::emit_jsx)
        .collect();
    assert_eq!(
        jsx_before, jsx_after,
        "CSS op must NOT change the JSX projection"
    );

    // Only document_css is dirty.
    assert!(proj.dirty.document_css, "css op marks document_css dirty");
    assert!(
        !proj.dirty.document_jsx && proj.dirty.sections.is_empty(),
        "no JSX dirtied"
    );
    ...
    assert!(
        css.contains("font-size: 14pt"),
        "css has the new size:\n{css}"
    );

    // Re-parse: the CharShape at index 1 reflects the edit (height 1400).
    let back = parse(&proj).unwrap();
    assert_eq!(
        back.char_shapes[1].height, 1400,
        "reparsed height = 14pt = 1400 HWPUNIT"
    );
...
fn t3_default_shape_omitted() {
    ...
    for rule in &proj.styles.rules {
        assert_ne!(
            rule.selector,
            hwp_jsx::css::Selector::Class("c0".into()),
            "no .c0 rule"
        );
        assert_ne!(
            rule.selector,
            hwp_jsx::css::Selector::Class("p0".into()),
            "no .p0 rule"
        );
    }
}
...
fn t5_raw_and_passthrough_byte_identical() {
    ...
    doc.passthrough
        .push("hwpx:source", vec![0u8, 1, 2, 255, 254, 0xC3, 0x28]);
            ...
            Inline::Note(NoteRef {
                ...
                body: vec![Block::Paragraph(Paragraph {
                    runs: vec![Run {
                        char_shape: 0,
                        char_ref: None,
                        content: vec![Inline::Text("주석 본문".into())],
                    }],
                    ..Default::default()
                })],
            }),
    ...
    assert!(
        doc_value_eq(&doc, &back),
        "Raw/Passthrough/inline doc lost content"
    );
    ...
    assert_eq!(
        back.passthrough.parts[0].bytes,
        vec![0u8, 1, 2, 255, 254, 0xC3, 0x28]
    );
}
...
fn t5b_unknown_inline_is_error_not_panic() {
    ...
    el.children.push(JsxNode::Element(JsxElement {
        tag_str: "Bogus".into(),
        ..Default::default()
    }));
    ...
    assert!(
        reparsed.is_ok(),
        "structural JSX parser tolerates unknown tags (never panics)"
    );
}
...
fn content_stats(doc: &SemanticDoc) -> Stats {
    ...
    s.perscript_fonts = doc
        .char_shapes
        .iter()
        .filter(|c| c.fonts.iter().any(|f| f.is_some()))
        .count();
...
fn t7_roundtrip_rich_hwp_corpus_via_lift() {
    ...
    for name in RICH {
        ...
        assert!(
            doc_value_eq(&doc, &back),
            "[{name}] doc_value_eq FAILED — structured codec lost content"
        );
        let fp1 = project_fingerprint(&proj);
        let fp2 = project_fingerprint(&emit(&back));
