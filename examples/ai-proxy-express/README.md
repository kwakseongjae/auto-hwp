# tf-hwp AI 프록시 — Express 템플릿 (issue 063)
...
화이트리스트는 전부 [`@tf-hwp/ai-protocol`](../../packages/ai-protocol) 이 소유한다(서버·클라 단일 출처).
...
const onAiRequest = async (instruction, anchors, ctx) => {
  const res = await fetch("http://localhost:8787/api/hwp-edit", {
    ...
    body: JSON.stringify({
      instruction,
      anchors,
      docContext: buildDocContext(ctx, anchors), // @tf-hwp/ai-protocol (R5 펜스)
    }),
  });
...
- `package.json` 의 `"@tf-hwp/ai-protocol": "file:../../packages/ai-protocol"` 를 **`"^0.0.1"`**(npm 발행본)으로 바꾼다.
