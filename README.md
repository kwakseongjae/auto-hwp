...
위 `askAi`가 부르는 `/api/hwp-edit`은 **당신 서버**입니다. 모델도, API 키도, 로그 보관 정책도, 한도도 당신이 소유합니다 —
우리 체험용 프록시는 이 경로에 전혀 관여하지 않습니다. 그대로 복사해 쓸 수 있는 얇은 서버가
[`examples/ai-proxy-express`](./examples/ai-proxy-express)에 있고, 프롬프트·검증·명령 화이트리스트는 `@auto-hwp/ai-protocol`이
클라이언트와 단일 출처로 공유합니다. 엔진 자체를 사내에 올리려면 [SELF-HOST](./docs/SELF-HOST.md)(Docker · Node/Bun).
...
| **렌더 · 좌표 질의** | 쪽마다 SVG 한 장을 돌려주고, 화면의 한 점이 어느 블록·표·셀·글자인지 되짚어 줍니다(커서 사각형 포함) | `renderPageSvgSanitized` · `hitTest` · `tableCellAt` |
| **구조화 편집** | 셀·문단 채우기, 표·문단·차트·이미지 삽입, 행 추가, 블록 이동과 삭제, 찾아바꾸기, 글자 서식, 표 열폭, 쪽 여백까지 — 모두 타입이 붙은 편집 명령(Intent)이고 하나가 되돌리기 한 단위입니다 | `applyIntent` · `undo`/`redo` |
...
| **양식 일괄 채움** | 양식과 채울 자리 정의에 명단을 얹으면 완성본 N부와 행별 검증 리포트가 나옵니다 | `auto-hwp inspect`/`fill` · [가이드](./docs/BULK-GUIDE.md) |
...
- **`.hwp`로 재저장 불가**: 저장 포맷은 HWPX입니다. HWPX 입력은 고치지 않은 영역이 바이트 그대로 보존되지만,
  `.hwp` 입력의 산출물은 **변환본**이라 쪽 나눔·표 너비가 달라질 수 있습니다. (한컴 "다른 이름으로 저장"이 뭉갠
  손실 `.hwpx`는 **레이아웃 정리** 모드가 열화를 감지해 원본 근사로 복원합니다.)
