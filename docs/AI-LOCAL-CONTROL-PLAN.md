...
> 권장 순서: **3-헤드리스(AI+MCP 코어) → 1(Tauri 뷰어 셸) → A를 셸에 임베드(라이브 제어) → 2(편집 심화) → 4 확장**. 이유: AI/op/MCP는 GUI 없이 코어에 먼저 붙여 검증할 수 있고(헤드리스 `auto-hwp ai-fill`), 그게 검증되면 Tauri 셸에 그대로 임베드된다.
...
- **에이전트 연결**: `claude mcp add --transport http auto-hwp http://127.0.0.1:<port>/mcp --header "Authorization: Bearer <token>"`.
...
- `tauri-plugin-single-instance`(2.3.6; `|app,argv,cwd|`로 두 번째 실행의 argv를 떠있는 인스턴스로 forward) + `tauri-plugin-deep-link`(`auto-hwp://` URI) + `tauri-plugin-cli`(argv 파싱). → `auto-hwp open doc.hwpx`로 창 포커스+로드.
...
- **A0 — 헤드리스 AI 코어 (GUI 불필요, 먼저) ✅ 완료**: `hwp-ai` `LlmProvider` trait + Anthropic(직접 HTTP, BYOK via env) + Mock. AST→Markdown projection(RAG). CLI `auto-hwp ai-fill <in.hwpx> --instruction "..." --out <out.hwpx>`(편집은 op-bus, export는 §M4). **AI가 검증된 .hwpx를 만든다(오라클 통과 확인).**
...
- `auto-hwp-app`(신규, Tauri 2): 뷰어 셸 + MCP HTTP 임베드 + single-instance/deep-link 편의 + BYOK 패널.
...
- `auto-hwp ai-context <in.hwpx>` — **read 툴**: `template_brief()`(준수할 스키마) + 문서 맥락(`to_markdown`) 출력. 코딩 에이전트가 이걸 읽고 JSON을 만든다.
- `auto-hwp ai-apply <in.hwpx> --content c.json [--out o.hwpx] [--verify]` — **write 툴**: parse→compile→apply→export(+오라클 verify).
