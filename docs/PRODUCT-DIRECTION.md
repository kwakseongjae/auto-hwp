# auto-hwp 제품 방향 v1 — "코어 하나, 셸 셋" (2026-07-02)
...
`hwp-ops`/`hwp-ai`/`hwp-mcp`)는 이미 헤드리스-우선으로 지어져 있다(`auto-hwp-cli`가 증명).
...
- **현실 주석 (2026-07-02, 017 이후)**: hwp-session은 렌더/지오메트리/DTO/export를 담당하는
  LEAF이고, **편집 레인(Session/Intent/apply_intent)은 hwp-mcp에 있다** — hwp-mcp의 HTTP
  서버(getrandom/std::net)를 `http` 피처(기본 on)로 게이트해 lib를 wasm-safe로 만들었다(이슈 **017**
  완료: `cargo check -p hwp-mcp --no-default-features --target wasm32-unknown-unknown` → exit 0,
  네이티브 셸 무변경). 세 셸은 "hwp-session(뷰) + hwp-mcp lib(편집)" 조합을 공유하고, wasm 셸은
  `open_bytes`/`export_bytes`(파일시스템 없는 바이트 표면)로 문서를 연다/내보낸다.
...
1. **게이트**: `benchmark.hwp`는 **8페이지 == 8페이지**를 유지해야 한다. 레이아웃을
   ...
   ```
   (98.9% 줄바꿈 정확도 이하로 떨어져도 실패로 간주.) `benchmarks/benchmark1.hwp`는 **18==18 (일치)** —
...
| P2b | [017](issues/017-hwp-mcp-wasm.md) | hwp-mcp wasm화 (http 피처 게이트 + open_bytes/export_bytes) | 012 |
