...
- **현실 주석 (2026-07-02, 012 이후)**: hwp-session은 렌더/지오메트리/DTO/export를 담당하는
  LEAF이고, **편집 레인(Session/Intent/apply_intent)은 hwp-mcp에 있다** — hwp-mcp의 HTTP
  서버(getrandom/std::net)를 `http` 피처로 게이트하면 lib가 wasm-safe가 되므로(실측: hwp-core는
  rhwp 포함 wasm 컴파일 통과), 세 셸은 "hwp-session(뷰) + hwp-mcp lib(편집)" 조합을 공유한다.
  게이트 작업은 이슈 **017**.
