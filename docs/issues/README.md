...
| [015](015-wasm-npm-package.md) | wasm npm 패키지 `@auto-hwp/engine` — HWP5 런타임 8p·골든 바이트동일·편집루프 wasm 실증 | **done** (PDF 글리프만 018) | P4-A | 4fb1ced · pkg/는 빌드산출물(gitignore, 레시피로 재빌드) |
...
| [016](016-react-component-library.md) | React 컴포넌트 라이브러리 `@auto-hwp/react` — sanitize 강제·mock 플로우 10테스트 | **done** | P4-B | 5a18459 · 브라우저 데모는 manual |
...
| [074](074-hwpx-page-undercount.md) | HWPX 입력 쪽수 과소 계산(본문상자·병합셀·ragged표·행높이) | **done** | P1 | 047fc14 ·  오라클 실측: benchmark1 .hwp 18==18 ✅ / 같은 문서 .hwpx 25 vs 우리 20 ❌, 한컴 저작 hwpx 4 vs 3. IR 은 파리티 오라클이 rhwp 와 일치 확인 → **조판 축**. 표 앵커 수정(59f784e)으로 가려져 있던 것이 드러남 |
...
1. [치명] `@auto-hwp/react`의 `file:` 상호의존 → 실버전 + 레지스트리 발행 순서(engine→editor-core→ai-protocol→react)
2. [치명] prepublish 빌드 훅 부재 → 빈 tarball 위험(engine=wasm-bindgen, react=vite+tsc)
3. [높음] 발행 CI·publishConfig(access:public) 부재
4. [높음] 비-Next 호스트용 wasm/워커 정적 서빙 레시피 부재(현 가이드 Next 편중)
5. [중] Next16 Turbopack 회귀·프레임워크 독립 AI 프록시 템플릿·published-package 임베드 예제 부재
...
방향(사용자 승인): 데스크톱 셸이 @auto-hwp/react HwpWorkspace를 소비 → UI 코드베이스 1개, R5~R8
...
| [026](026-editor-core-sdk.md) | @auto-hwp/editor-core + @auto-hwp/ai-protocol — headless SDK 재배치 | **done** | R3-P1 | core 20테스트(node)·protocol 13(프롬프트 바이트동일)·vanilla 증명·e2e 무수정 통과 |
...
> 완료된 완성형 핵심(글자/문단/글꼴/표 병합·음영/목록)은 `docs/COMPLETION-ROADMAP.md`와 `docs/history/CHECKLIST.md` 참고.
