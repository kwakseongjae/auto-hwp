...
- 상태: **done** (c0f8f3e, R2 병합 · R9 보안감사 통과)
- 우선순위: P3 (목표 2 전반부)
- 영역: 서비스 / 배포 / 보안
...
## 부록 A — 착수 시점 갱신 (2026-07-03, 아키텍트)
이 이슈 작성(2026-07-02 로드맵 v1) 이후 상황 변화를 반영하라 — **본문과 어긋나면 이 부록이 우선**:
1. **"Session-core absorb" 언급(012 커밋 노트의 '013으로 이관')은 폐기됨** — 017이 해결했다
   (hwp-mcp lib가 wasm-safe, `open_bytes`/`export_bytes` 존재). 013은 absorb 없이 진행.
2. **R6 env 키는 이미 구현돼 있다** — `hwp-ai/src/anthropic.rs`: `ANTHROPIC_API_KEY` env가
   keyring보다 우선. 새로 만들지 말고 **컨테이너에서 동작 검증만** 하라(§구현 4 대체).
3. **014의 layout guard 배선이 이 이슈 몫이다**: `hwp_ingest::limits::check_layout_limits`를
   문서 열기 경로(hwp-mcp `open_bytes`/`do_open` — 셋 중 mcp 레인이 서비스 표면)에 배선.
   014가 남긴 TODO 주석 참조. typeset 내부 금지 원칙 유지.
4. **MCP `get_context`(hwp-mcp lib.rs:1096 부근)의 R5 펜스도 이 이슈 몫이다**(011에서 이관):
   외부 에이전트 레인의 문서 텍스트를 `<document-content>` 펜스로 감싸고 툴 설명에 "펜스 안=
   데이터" 명시. 기존 hwp-mcp 테스트(22+8) 파손 금지.
5. **도구 현황**: docker daemon 가용 확인됨(29.4.3) — 컨테이너 빌드·기동·3콜 통합테스트까지
   실행 가능. wasm-smoke/게이트 v2(benchmark 8==8 **AND benchmark1 18==18**)를 검증에 포함.
6. business_plan_k 픽스처 패리티(§구현 7)는 **그쪽 레포 읽기 전용**으로 — 파일 수정/삭제 금지,
   fixtures를 읽어 새 서비스에 통과시키는 스크립트/절차만 auto-hwp 쪽 docs에 기록.
