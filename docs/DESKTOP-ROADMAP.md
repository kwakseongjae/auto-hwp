...
범위·인증·벤치·rhwp 감시의 정본은 이슈 **#331**이다. 이 절은 그 이슈를 로드맵에 고정한다.
...
- 인증: Anthropic·OpenAI·xAI **API 키를 키체인에**. Claude/ChatGPT/Grok 소비자 구독 OAuth는
  0.1에 넣지 않는다(서드파티 계약이 없거나 약관이 금지).
...
**#331의 1단계: 이 절(0.2)을 유지한 채, 기능 태그 50문서와 네 동사 픽스처를 만든다.
`App.tsx` 조각 수정, 구독 OAuth, rhwp vendored 수정은 하지 않는다. #144 서명·업데이터는
0.1 배포 단계에서 기존 계약대로 간다.**
