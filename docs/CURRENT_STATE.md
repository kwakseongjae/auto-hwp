...
- 갱신: 2026-09-21 · Claude Opus 5 — **#307 머지 · #305 수리(PR 대기)** (main `32958c2`).
  ...
  · **#305 — 게이트가 왜 늘 빨갰나: 부분 갱신이 쌓인 것이다.**
    `복학원서` 항목은 **최초 baseline(#250) 이후 한 번도 다시 쓰이지 않았고**,
    #275 는 baseline 을 **11줄만** 고치며 이 항목을 건드리지 않았다.
    · **폰트 가설은 틀렸다**(내가 #305 본문에 그렇게 썼다가 정정) — 번들 `NanumGothic` 이
      `FONT_CANDIDATES` **1순위**라 기계 독립이고, 폰트를 안 보는 approx 빌드도 같은 17줄.
      **스윕이 `target/release/auto-hwp` 를 `rhwp,shaper` 로 덮어써서** 여태 구분이 안 됐다 —
      두 빌드를 **다른 `CARGO_TARGET_DIR`** 로 따로 만들어야 갈린다.
    · **고친 것**: ① baseline 에 **생성 커밋 SHA**(더러우면 `-dirty`) — 어긋났을 때
      「다른 기계인가 다른 코드인가」를 가른다 ② **재채점 `--check` 를 CI 로** 올렸다
      (여태 로컬 `--full` 의 마지막 단계라 넘겨 보기 쉬웠다) ③ baseline 재생성
    · **쌓여 있던 드리프트는 순증이었다**: `mean_line_exact` 97.4 → **97.6** ·
      `mean_cell_exact` 96.7 → **97.2** · 일치 53 → **54**. 좋아짐 21 · 나빠짐 9.
      **게이트가 빨간 이유가 「회귀」가 아니라 「개선을 못 따라간 것」이었다.**
    · 재생성이 잠그는 **하락 9건은 #311 로 분리**했다 — 가장 큰 것은 `복학원서` 가
      한컴보다 **1줄 덜 접히는 것**(`our_lines 18 → 17`, #250~#275 사이에 생겼다)
  · **열린 것**: #311(하락 9건) · #247(단일 지렛대 없음) · #258(rhwp 서브모듈, 사용자가
