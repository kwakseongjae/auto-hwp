...
## 지금 (현재 위치)
- 로드맵 기준: **R12 + R13 완료, R14 062 quick win 완결** — R12(051~057), R13(058·059·060),
  R14 062-1 배포용복호(056해소)·062-2 옛한글·062-3 금칙 전부 병합·검증(verify-local --full 그린, e2e 39/39).
- **다음 = 063 웹 이식 패키징**(npm 발행 준비 → 외부 사이트 임베드). 062 잔여(대각선·수식·폰트메트릭·차트)는 R14 후속.
