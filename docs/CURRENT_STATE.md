...
- 갱신: 2026-09-20 · Claude Opus 5 — **#303 머지 · #302 수리 완료(PR 대기)** (main `f601a54`).
  · **#302 원인**: 아무것도 그리지 않는 줄이 쪽을 넘기고, 곧바로 강제개쪽이 오거나 문서가
    끝나 **그 쪽이 통째로 빈다.** 문서 78건 중 **7건**이 글자까지 같은 모양.
  · **#302 는 쪽수 지렛대가 아니라 눈에 보이는 결함이다** — `prism-moel` 은 13쪽 보고서의
    **2쪽이 백지**였다. 빈 쪽을 전부 없애도 총 격차는 `−1` 뿐이다(쪽이 모자란 문서에도 있다).
  ...
  · **최종 규칙은 좁다** — 일곱 건은 전부 **쪽이 이미 100~112% 로 넘친 뒤** 빈 줄이 왔다:
    ...
    **세 경로(`NaiveLayout`·`place_doc`·`block_pages`)가 이 함수 하나를 부른다**(불변식 2).
    ...
    | 내부 문서 20건 격차 | 5 | 1 | **3** |
    | 내부 문서 일치 | 16 | 19 | **18** |
    | corpus/private 56건 | 44 | 46 | **43** |
    | 캐럿 계약 | ✅ | ❌ | **✅** |

    움직인 문서는 4건뿐 — `prism-moel +3→-1` · `synth-86p +1→0` · `synth-146p -13→-14` ·
    `bizinfo-mss +3→+4`. 문단·셀 적중률은 **안 떨어졌다**(`page_match` 31→32 · 셀 정확 +6).
    남은 빈 쪽 1건(`bizinfo 별지1` 26쪽)은 `남음 968` 로 **여유 있는 쪽**이라 다른 원인이다.
  · **내가 앞서 세 가지를 틀렸고 #302 에 정정했다**:
    ① 「extract-text 가 끝의 빈 줄을 보여준다」 → **`cat -A` 는 macOS 에 없다.** 근거 무효
    ② 「`필요 1200` = 빈 문단」 → **아니다.** 어스샷 4·5쪽도 1200인데 내용이 있다.
       빈 쪽의 유일한 증거는 **`요소 0`**
    ③ 「격차의 17% 회수」 → **40건 부분집합의 우연**
  · **점수판 스크립트를 이번엔 저장했다**(전에 유실): 스크래치패드 `board2.sh`(총 쪽 격차) ·
