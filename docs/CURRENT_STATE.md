...
- 갱신: 2026-09-22 · Grok 4.7 — **데스크톱 0.1 범위를 #331로 고정** (문서 PR, 구현 전).
  · 출시 정의: 채팅이 문단 수정·표 추가·다음 페이지·PDF를 하고, 사람은 페이지에서 제안 카드를 본다.
    구독 OAuth(Claude/ChatGPT/Grok)는 넣지 않는다. 키체인 API 키 세 장.
  · 9월 21일 「개별 데스크톱 이슈를 깊이 파지 않는다」는 그대로다. #270·#144·#139는 #331 단계 안에서만.
  · 다음 구현은 `App.tsx`가 아니라 **기능 태그 50문서 + 네 동사 녹화 테스트**.
