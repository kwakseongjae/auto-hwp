...
## 지금 (현재 위치)
- 로드맵 기준: **R12 완료 + R13 거의 완료** — R12(051~057) 전부 병합. R13: 059 IME(1ea3365)·058 폰트(43a7c48)
  병합·검증 완료(verify-local --full 그린: 게이트 8==8·18==18, vitest 156+15+296+41, e2e 39/39).
  **R13 잔여 = 060 프레임표 하나.** 058은 "디스플레이 전용"으로 게이트 무영향(metric 불변) 증빙.
...
## 다음 (즉시 착수 가능 — 사용자 승인 대기)
기본 순서(사용자 브리핑 제안): **060 프레임표(R13 마감)** → **062 quick win 배치**(062-1 배포용복호화=056 해소 ∥
062-2 옛한글 ∥ 062-3 금칙) → **063 웹 이식 패키징**. 사용자가 순서 조정 가능(062 병렬/063 우선 옵션 제시함).
