...
- 갱신: 2026-09-22 · Grok 4.7 — **0.0.3 오른쪽 독** (#336). 사용자가 ㄱㄱ 한 뒤 시작.
  · 문서를 열면 오른쪽 독이 열린다. 데모 안내 배너는 없다. 제안 카드는 그대로다.
  · 버전은 0.0.3. 공개 다운로드와 0.1.0 공증은 바꾸지 않는다.
  · **다음 ㄱㄱ 전에는 0.0.4를 시작하지 않는다.** 0.0.4는 모델 설정과 Claude 구독 연결이다.
