...
- 갱신: 2026-09-20 · Claude Opus 5 — **#303 머지 · #302 수리는 설계 결정 대기** (main `f601a54`).
  · **#302 원인 확정**: 아무것도 그리지 않는 줄이 쪽을 넘기고, 곧바로 강제개쪽이 오거나
    문서가 끝나 **그 쪽이 통째로 빈다.** 문서 78건 중 **7건**이 글자까지 같은 모양.
  · **내가 앞서 세 가지를 틀렸고 #302 에 정정했다**:
    ① 「extract-text 가 끝의 빈 줄을 보여준다」 → **`cat -A` 는 macOS 에 없다.** 근거 무효
    ② 「`필요 1200` = 빈 문단」 → **아니다.** 어스샷 4·5쪽도 1200인데 내용이 있다.
       빈 쪽의 유일한 증거는 **`요소 0`**
    ③ 「격차의 17% 회수」 → **40건 부분집합의 우연.** 78건으로 재면 빈 쪽을 전부 없애도 **−1**
  · **#302 는 쪽수 지렛대가 아니라 눈에 보이는 결함이다** — `prism-moel` 은 13쪽 보고서의
    **2쪽이 백지**다. 쪽수가 맞든 틀리든 잘못된 출력이다.
  · **수리는 됐다**(`fix/302-blank-line-page` `0ba97b7`, 푸시됨). 공유 판정 한 줄
    `line_draws_something(ls) = ls.horz_size > 0.0` 을 **세 경로가 함께 부른다**(불변식 2).
    **빈 쪽 7 → 0 · 내부 문서 20건 격차 5 → 1 · 일치 16 → 19.** 어스샷은 안 나빠졌다 —
    쪽을 지우는 게 아니라 **이른 break 를 없애 진짜 내용이 끊게** 하기 때문.
    corpus/private 56건은 44 → 46(합성 제외 31 → 32).
  · **그런데 게이트가 빨갛고 그게 옳다.** `hwp-session::body_caret_whitespace_keeps_forced_lines…`
    이 **「빈 줄은 자기 쪽을 갖는다」를 계약으로 박아 두었다** — 캐럿이 그 줄에 앉아야 하므로.
    오라클 계약이 아니라 **우리가 스스로 정한 동작**이지만 한컴과 정면으로 부딪힌다.
    **A** 계약을 바꾼다(캐럿이 아래 여백에) · **B** 안 고친다(백지 유지) ·
    **C** 캐럿을 쪽 안으로 클램프한 뒤 A. → **사용자 결정 대기. PR 은 올리지 않았다.**
