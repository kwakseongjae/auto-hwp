...
- 갱신: 2026-09-22 · Grok 4.7 — **0.0.1을 실기에서 보고 버전 열차를 다시 씀**.
  · 로컬 0.0.1로 공개 benchmark.hwp를 열면 8쪽·표가 보인다. 첫 화면은 단정하고, 편집 크롬이 두 줄이다.
    채팅은 목업, 상태는 AI 없음.
  · 구독 연결은 넣는다. 이 맥 Aside 1.0.914의 Models에 Claude Subscription이 연결되어 있다.
    Anthropic 법률 문서는 서드파티 Pro/Max 사용을 금지한다고 여전히 적는다. 구현은 연결 팝업이지 자격 증명 스크랩이 아니다.
  · 버전은 0.0.2부터 0.0.1씩. 각 버전의 보고는 목표·한 일·화면. **0.1.0 공증은 사용자 승인 후.**
    다음 구현은 **#334** 도구막대 한 줄. 열차는 **#333**.
    **사용자가 ㄱㄱ 하기 전에는 다음 버전을 시작하지 않는다.** 각 버전이 끝나면 목표·한 일·화면을 보여주고 다음 버전만 브리핑한다.

- 갱신: 2026-09-22 · Grok 4.7 — **데스크톱 0.1 범위를 #331로 고정** (문서 PR, 구현 전).
  · 출시 정의: 채팅이 문단 수정·표 추가·다음 페이지·PDF를 하고, 사람은 페이지에서 제안 카드를 본다.
  · 인증·다음 구현은 **바로 위 항목이 정본**이다. 이 항목의 구독 제외는 실기 확인 전에 적은 것이고 취소됐다.
  · 9월 21일 「개별 데스크톱 이슈를 깊이 파지 않는다」는 그대로다. #270·#144·#139는 버전 열차 안에서만.
...
- 갱신: 2026-09-21 · Claude Opus 5 — **데스크톱은 별도 라운드로 뺀다** (main `d8221c8`).
  · **사용자 결정**: 데스크톱 앱은 **`astra` 와 함께 한번 대대적으로 손본다.** 그 전까지
    **데스크톱 이슈를 개별로 깊이 파지 않는다** — 하나씩 고치면 큰 재설계와 충돌한다.
    같은 줄: **#270**(열기 영구 정지) · **#271**(pnpm/npm 문서) · **#139**(로드맵) · **#144**(서명 설치본).
    `astra` 가 무엇/누구인지는 말하지 않았다 — **추측해서 쓰지 말 것.**
  · **#328 머지** — 열기가 8초를 넘으면 **무엇을 기다리는지 말하고 「기다리지 않기」**를 준다.
    **끊지 않는다**(큰 문서는 정말 느릴 수 있어 하드 타임아웃은 정상 동작을 깨뜨린다).
    취소는 기다림만 포기하고 늦게 온 결과는 **토큰으로 버린다** — 다른 문서를 새로 열었을 때
    덮어쓰면 안 되기 때문. 출시 빌드는 `VITE_SHELL` 없이 나가므로 **레거시 `App` 셸**이 그 경로다.
  · **#270 은 열려 있다 — 원인 미규명.** 정적 감사로는 보호 폴더 접근이 **안 보인다**
    (`SourceIdentity::read` 는 연 파일만 · 최근 문서는 `app_data` 안에서만).
    ⚠️ 이슈 본문의 **「Rust 가 명령을 받지 못했다」는 추론이 약하다** — `open_doc` 은
    `spawn_blocking` 이라 **메인 스레드 샘플에 원래 안 나온다.** 그 관찰만으로 세울 수 없다.
    재현은 TCC 동의창을 띄워 둔 채여야 해서 **사용자만 가능**하다.
  · **다음 후보(데스크톱 제외)**: **#247 다음 계측기** — 남은 `맨EM` 12.71쪽(예약의 74%).
    렌더 baseline 간격을 **문단 단위로** 맞대야 한다. `--rendered` 는 쪽당 글자까지만 본다.
    · 사용자만 가능: **#258**(포크 태그 `auto-hwp-v0.8.6-p1`) · **#274**·**082**(한/글 설치)

- 갱신: 2026-09-21 · Claude Opus 5 — **격차에 단일 축이 생겼다** (main `1a4c309`).
  · **`layout-check --rendered` (#321)** — 저장 `lineseg` 가 제거된 문서를 **rhwp 가 그린 것**
    으로 잰다. 쪽마다 **공백 아닌 글자 수**를 양쪽에서 같은 방식으로 센다.
    `benchmark1.hwpx`: **총 글자 17184 = 17184 인데 쪽당 −16.7%** — 그것이 30쪽 vs 25쪽이다.
  · **줄 수로 재려다 버렸다.** 「서로 다른 baseline 개수」는 줄 수가 **아니다** — 나란한 셀이
    같은 baseline 을 공유해 합쳐지고 **두 엔진이 다르게 합친다**. 실측: `복학원서.hwp` 는
    저장 오라클 **100% 정확**인데 baseline 으로는 `우리 39 · 한컴 60`. **아는 답이 있는
    문서로 먼저 검증한 덕에 잡았다.** 쪽수가 맞는 문서에서 `0.0%` 가 나오는지가 검증 기준이다.
  · **쪽 격차가 있는 문서는 전부 같은 방향**(쪽당 덜 담는다)이고 크기가 격차와 같이 움직인다:
    `04_아이디어_기획서` 3/3 → **0.0%** · `별첨02` 32/31 → −3.8% · `과천시` 9/8 → −11.1% ·
    `benchmark1` 30/25 → **−16.7%** · `prism-moel` 12/10 → −20.3%(총 글자 0.956 ⚠).
  · **줄간격 가설은 부분적으로만 맞았다**(#247 에 기록). 퍼센트 줄간격에 계수를 넣어 쓸어 보니
    `0.875` 로 **30 → 28** 까지만 가고 `0.80` 으로도 27 이다. **5쪽 중 2쪽 몫**이다.
    남은 것은 행 예약의 다른 항(패딩 `280 × 215행 = 1.37쪽` 등) — 다만 셀 여백은 #281 이
    HWP 기본값으로 이미 교정한 값이라 그냥 줄이면 회귀다.
  · **#319 머지**: 줄수 채점이 `display_paper` 의 가로 전환을 안 타고 본문 폭을 손계산했다
    (조판 72850 · 채점 48190). `benchmark1` `line_exact 89.5 → 92.9` · `±1 98.2 → 99.7`.
    진단 두 곳(`--rows` 폭 · 요약 높이)도 같은 손계산이라 함께 고쳤다.
  · **#249 는 성격을 다시 썼다**: 원래 지목한 `#206` 과대 셀 경로는 **끄고 재어 0쪽** 확인.
    제목도 「가로 문서 꼬리 구간」으로 바꿨다가, 계측기로 보니 **꼬리가 특별한 게 아니라
    쪽당 밀도가 문서 전체에서 낮은 것**이라 그 관찰은 취소했다.
  · **항 분해를 끝냈고 후보 다섯이 전부 닫혔다**(#247 에 근거 기록). `benchmark1.hwpx`
    표 215행 전수: `reserved 17.11쪽 = 맨EM 12.71 + 줄간격 1.11 + 셀패딩 1.37 + (나머지 1.92)`.
    나머지 1.92쪽은 `row_span` 분할 + **저장 행 높이 바닥**이다.

    | 후보 | 결과 |
    | --- | --- |
    | `#206` 과대 셀 연속 | **0쪽** — 끄고 재어 확인 |
    | 줄간격 ×0.875 (글꼴 줄높이 가설) | **5쪽 중 2쪽** (30→28) |
    | 저장 행 높이 **바닥 끔** | 3쪽(30→27)이지만 **corpus 42 → 165** |
    | 스텁 판정(저장 높이 전부 같음)으로 좁히기 | **corpus 42 → 140** |
    | `.hwpx` 가 `noAdjust` 를 못 읽는다 | **아니다** — `noAdjust=0` 을 제대로 읽는다 |

    ⚠️ 바닥 실험에서 **일치는 36 → 40 으로 올라가는데 총 격차는 42 → 165** 였다.
    **이진 지표만 봤으면 「개선」으로 읽었다** — `page-delta.sh` 가 총 격차를 쓰는 이유다.
  · **「단일 지렛대 없음」이 이제 관찰이 아니라 측정이다.** 남은 것은 `맨EM` 12.71쪽
    (예약의 74%) — 줄 자체가 두꺼운지 보려면 렌더 baseline 간격을 **문단 단위로** 맞대야 한다.
    `--rendered` 는 쪽당 글자까지만 본다. **그것이 다음 계측기다.**

- 갱신: 2026-09-21 · Claude Opus 5 — **백지 쪽 0건 달성** (main `4f9568c`).
  · **`shaper` 정본 실측**: 백지 쪽 **0** · 내부 문서 20건 격차 **3**(일치 17) ·
    corpus/private 56건 격차 **42**(일치 36) · 오라클 게이트 **리눅스 CI 초록**.
  · **「아무것도 그려지지 않은 쪽을 만들지 마라」가 세 얼굴로 갈라졌고 전부 잡았다.**
    판정은 `line_forces_break` **하나**에 모여 있다(불변식 2):

    ```rust
    if ls.horz_size > 0.0 { return true; }   // 그리는 줄은 언제나 끊는다
    if trailing { return false; }             // #314 — 마지막 그리는 블록보다 뒤
    vert < body_h                             // #302 — 이미 넘친 쪽
    ```

    · **#302** 넘친 쪽에 온 빈 줄 · **#307** 빈 문단 뒤 강제 개쪽(가드를 「그려진 게 있나」로) ·
      **#314** 문서 끝의 빈 문단
  · **#314 의 요점은 캐럿 계약을 피한 방식이다.** `section_last_drawing_block` 이 `None`
    이면 규칙이 **아예 발동하지 않는다** — 「뒤따르는 빈 줄」은 앞에 무언가 있어야 성립한다.
    `body_caret_whitespace_…` 의 문서(`"\n\n"` 하나)는 그리는 블록이 없어 그대로 2쪽이다.
    #302 에서 「빈 줄은 절대 쪽을 못 넘긴다」로 넓혔다가 이 계약이 깨졌었다 — 이번엔
    **경계를 규칙의 정의 안에** 넣었다. 4/4 통과.
  · **쪽수가 아니라 백지 유무로 판정했다.** `어스샷` 은 `0 → −1` 이 됐지만, 우리 6쪽이
    비어 있고 **한컴은 6쪽 전부 내용**이라(앵커 55개 실측) 내용 쪽은 원래 `5 vs 6` 이었다 —
    백지가 부족을 가리고 있었을 뿐이다.

- 갱신: 2026-09-21 · Claude Opus 5 — **#305 머지 · 계측 정본은 `shaper` 다** (main `504cfad`).
  · ⚠️ **이 세션의 보드 숫자는 대부분 `approx` 기준이었다.** 오라클 게이트는 `shaper` 로
    채점하므로 **그쪽이 정본**이다. 같은 main·같은 문서인데 갈린다:
    백지 쪽 approx **0** · shaper **1** · corpus 총 격차 44 · **42** ·
    `복학원서` 우리 줄수 17 · **18**.
  · **shaper 기준 현재 상태**: 백지 쪽 **1건**(`어스샷` 6쪽) ·
    내부 문서 20건 격차 **2**(일치 18) · corpus/private 56건 격차 **42**(일치 36).
  · **#305 의 진짜 원인**: `oracle-sweep.mjs` 의 `ensureCli()` 가 `target/release/auto-hwp`
    가 있으면 **기능을 확인하지 않고 그대로** 썼다. 내가 #302·#307 측정용으로 만든
    `--features rhwp`(shaper 없음) 빌드를 스윕이 쓰고 있었다.
    올바른 빌드로 재니 **옛 baseline 과 정확히 일치** — **드리프트도 기계 의존도 회귀도 없었다.**
    · **내가 세 번 오진했다**(전부 #305 에 정정): ① 기계·폰트 차이 ② 부분 갱신이 쌓였다
      ③ 리눅스 의존 — 셋 다 **approx 빌드로 잰 것이 근거**였다. **CI 가 옳고 내 로컬이 틀렸다**
      (CI 는 바이너리가 없어 제대로 빌드한다). 그 전제로 연 **#311 은 닫았다.**
    · **고친 것(PR #312 머지)**: ① `ensureCli()` 가 **언제나 빌드** ② `layout-check --json` 이
      **`metrics: shaper|approx`** 를 싣는다 ③ baseline 이 `metrics` + **생성 커밋 SHA** 를
      기록하고 `metrics !== "shaper"` 면 **거부** ④ 재채점 `--check` 를 **CI 로** 올렸다.
      주입 2/2 🔴. **리눅스 CI 에서 `--check` 초록** = 기계 독립이 증명됐다.
    · baseline 재생성: 82건 중 **81건이 이전과 동일**, 바뀐 하나는 `2015년_재난안전…hwpx`
      `our_pages 82 → 77` — **#302·#307 이 낸 진짜 개선**.
  · **#307 종결(PR #310)**: 그려진 것이 없는 쪽에서는 **강제 개쪽이 무동작**이다.
    기존 가드 `brk[i] && vert > 0.0` 가 이미 그 뜻이었는데 **빈 문단이 `vert` 만 올려** 무력화했다.
    리셋이 필요 없는 「마지막으로 그린 쪽 번호」로 든다. 「그렸다」 정의는 세 경로가 같아야 한다
    (`horz_size > 0` 인 줄 또는 표의 행) — **글리프 유무로 재면 갈린다**.
  · **진단기에 끊은 블록 번호를 실었다** — `blocks` 는 표 구간이라 **백지 쪽에서는 언제나 비어**
    있어서 「무엇이 이 쪽을 만들었나」를 물을 대상이 없었다.
  · **열린 것**: **#314**(shaper 기준 백지 1건 — 문서 끝의 빈 줄인데 **쪽에 여유가 있는**
    세 번째 변종. #302 는 「넘친 쪽」, #307 은 「다음이 강제 개쪽」이라 둘 다 안 닿는다.
    넓히면 캐럿 계약이 깨진다) · **#313**(계측기도 같은 함정 — PR #315) ·
    #247 · #258(사용자가 포크 태그 생성 필요) · #274(한/글 필요) · #270/#271 · #249

- 갱신: 2026-09-21 · Claude Opus 5 — **#307 머지 · #305 수리(PR 대기)** (main `32958c2`).
  · **#307 종결(PR #310)**: **그려진 것이 없는 쪽에서는 강제 개쪽이 무동작**이다.
    기존 가드 `brk[i] && vert > 0.0` 가 이미 「백지 쪽을 만들지 마라」였는데
    **빈 문단이 아무것도 안 그리면서 `vert` 만 올려** 무력화했다. 가드를 「그려진 게 있나」로
    바꾸되 **리셋이 필요 없는 형태**(「마지막으로 그린 쪽 번호」)로 들었다 — 쪽마다 플래그를
    껐다 켜면 초기화 지점 하나를 빠뜨렸을 때 조용히 틀린다. 「그렸다」의 정의는 세 경로가
    같아야 한다(`horz_size > 0` 인 줄, 또는 표의 행) — **글리프 유무로 재면 갈린다**.
    **백지 쪽 1 → 0 · 내부 문서 격차 3 → 2.** 예측 밖 개선: `별첨02` `+2 → +1`
    (백지가 없던 문서인데 빈 문단 뒤 강제개쪽이 쪽을 한 장 버리고 있었다).
  · **진단기에 끊은 블록 번호를 실었다** — `blocks` 는 표가 놓인 구간이라 **백지 쪽에서는
    언제나 비어** 있어서 「무엇이 이 쪽을 만들었나」를 물을 대상이 없었다. 배선하니 바로 나왔다.
  · **#305 — 게이트가 빨갰던 진짜 원인: 스윕이 남의 빌드를 쓴다.**
    `oracle-sweep.mjs` 의 `ensureCli()` 가 `target/release/auto-hwp` 가 있으면 **기능을
    확인하지 않고 그대로** 썼다. 내가 #302·#307 측정용으로 만든 `--features rhwp`
    (shaper 없음) 빌드가 거기 있었고, 스윕이 그걸로 채점했다.
    **같은 문서·같은 코드인데 둘은 다른 숫자를 낸다**:
    `복학원서` shaper **18줄(100%)** · approx 17줄(94.1%) ·
    `benchmark1` shaper **969셀줄(98.5%)** · approx 973(98.9%).
    올바른 빌드로 재니 **옛 baseline 과 정확히 일치** — **드리프트도 기계 의존도 회귀도 없었다.**
    · **내가 세 번 오진했다**(전부 #305 에 정정): ① 기계·폰트 차이 ② 부분 갱신이 쌓였다
      ③ 리눅스가 옛 값을 내므로 기계 의존 — **셋 다 approx 빌드로 잰 것이 근거였다.**
      CI 는 바이너리가 없어 제대로 빌드하므로 **CI 가 옳고 내 로컬이 틀렸다.**
    · **고친 것(PR #312)**: ① `ensureCli()` 가 **언제나 빌드** ② `layout-check --json` 이
      **`metrics: shaper|approx`** 를 싣는다 ③ baseline 이 `metrics` + **생성 커밋 SHA** 를
      기록하고 `metrics !== "shaper"` 면 거부 ④ 재채점 `--check` 를 **CI 로** 올렸다.
      주입 2/2 🔴.
    · **재생성 결과: 82건 중 81건이 `main` 과 동일.** 바뀐 하나는
      `2015년_재난안전…hwpx` 의 `our_pages 82 → 77` — **#302·#307 이 낸 진짜 개선**이다.
    · ⚠️ 앞서 내가 올린 「드리프트가 순증(97.4 → 97.6)」과 그 전제로 연 **#311 은 무효**다
      (approx 와 shaper 를 견준 것). **#311 은 닫았다.**
  · **열린 것**: #247(단일 지렛대 없음) · #258(rhwp 서브모듈, 사용자가
    포크 태그 생성 필요) · #274(셀 폭 규칙, 한/글 필요) · #270/#271(데스크톱) · #249

- 갱신: 2026-09-20 · Claude Opus 5 — **#302·#303 머지 · 백지 쪽 7 → 1** (main `9232b22`).
  · **#302 종결(PR #306)**: 아무것도 그리지 않는 줄이 **이미 넘친 쪽**을 또 넘겨 백지 쪽을
    만들던 것. `prism-moel` 은 13쪽 보고서의 **2쪽이 백지**였다.
    `line_forces_break(ls, vert, body_h) = ls.horz_size > 0.0 || vert < body_h`
    — **세 경로(`NaiveLayout`·`place_doc`·`block_pages`)가 이 함수 하나를 부른다**(불변식 2).
  · **main 실측**: 백지 쪽 **7 → 1** · 내부 문서 20건 격차 **5 → 3**(일치 16 → 18) ·
    corpus/private 56건 **44 → 43**. 문단·셀 적중률은 **안 떨어졌다**(`page_match` 31→32 · 셀 정확 +6).
  · **규칙을 한 번 넓게 잡았다가 게이트가 잡아 줬다.** 「빈 줄은 절대 쪽을 넘기지 않는다」는
    `hwp-session::body_caret_whitespace_keeps_forced_lines…` 를 깼고 **그 계약이 옳았다** —
    여유가 남은 쪽(1700 중 1200, 70%)에서는 빈 줄도 쪽을 넘겨야 캐럿이 그 줄에 앉는다.
    넓은 규칙은 corpus 를 **46 으로 악화**시키기도 했다. **좁힌 쪽이 두 판 모두에서 낫다.**
  · **내가 이 조사에서 네 번 틀렸고 전부 #302 에 정정했다**:
    ① 「extract-text 가 끝의 빈 줄을 보여준다」 → **`cat -A` 는 macOS 에 없다.** 근거 무효
    ② 「`필요 1200` = 빈 문단」 → **아니다.** 요소 469개인 쪽도 1200 이다.
       **백지의 유일한 증거는 `요소 0`**
    ③ 「격차의 17% 회수」 → 40건 부분집합의 우연. 78건으로 재면 **−1**
    ④ 「A·B·C 중 골라 달라」 → **트레이드오프가 실재하지 않았다.** 규칙이 너무 넓었을 뿐
  · **#305 — 게이트가 신호를 못 주고 있었다.** `oracle-sweep --check` 가 빨간데
    `main` · `e71b462`(#287) · **`370ec0f`(baseline 을 쓴 바로 그 커밋)** 전부 **같은 10건**이다.
    스윕이 `rhwp,shaper`(실제 폰트 advance)로 채점하는데 baseline 에 플랫폼이 없다.
    **회귀가 아니라 도구 문제.** ⚠️ 「#287: --full 그린 · baseline 동일」이라는 **내 기록은 틀렸다.**
    당분간 `--full` 을 돌리면 **`main` 과 출력을 대조**해야 새 회귀를 안다.
  · **계측기를 레포에 넣었다**(#308) — 전에 스크래치패드에만 있다가 **잃어버렸다**:
    `scripts/page-delta.sh`(총 쪽 격차) · `scripts/empty-pages.sh`(백지 전수).
    **코퍼스는 둘이다** — 내부 문서 `../business_plan_k/fixtures/kstartup-forms`(최종 목표) ·
    회귀 감시 `corpus/private`(합성 `synth-146p` −13 이 지배). `corpus/hwp`·`benchmarks` 는
    둘 다에 없고 `oracle-sweep` 이 본다 — #302 때 **게이트가 그걸 먼저 봤다**.
  · **열린 후속**: **#307**(남은 백지 1건 — `bizinfo 별지1` 26쪽, `남음 +968` 로 **여유 있는
    쪽**이라 원인이 다르다) · **#305**(게이트 재현성) · #247(단일 지렛대 없음) ·
    #258(rhwp 서브모듈 — 사용자가 포크 태그 생성 필요) · #274(셀 폭 규칙, 한/글 필요)
  · **#303 머지**(`Closes #304`): `--pages` 가 채점과 같은 `RealFontMetrics` 를 쓴다.
    **`on: pull_request` 에 `types:` 가 없으면 `edited` 가 빠진다** — 본문만 고치면 CI 가
    다시 안 돈다. 커밋을 밀어야 한다.

- 갱신: 2026-09-20 · Claude Opus 5 — **#247 쪽수 조사: 단일 지렛대는 없다** (main `a9c036c`).
  · **제보자 문서 40건 전수 채점**: 일치 31/40 · **총 쪽 격차 18**(전부 초과 방향).
    `layout-check` 가 한컴 저장 `lineseg` 로 채점하므로 정답지 PDF 없이도 전부 잰다.
  · **크기는 맞는다**: 본문 줄수 −0.2% · 셀 줄수 +0.5% · 행 높이 **+0.2%(0.04쪽 분)**.
    `+0.5%` 로는 `+15%` 의 쪽을 못 만든다.
  · **지운 축**: 문단 여백 축척 · 표 폭 스트레치 — **둘 다 실제로 고쳐 보고 효과 0**
    (총 격차 18 불변). 강제개쪽 과다 아님(문서 20·우리 21). `keep_table_on_fresh_lane` 은
    발동 조건상 불가.
  · **진짜 성격**: 내용 기인 break **109개 중 77개(70%)가 본문 5% 안쪽**, 22개는 1% 안쪽으로
    아슬하게 넘친다. **경계가 칼날이라 0.3% 도 쪽을 하나 만든다.**
  · **예산**: 본문 높이 `+1%` → 4쪽 회수 · `+5%` → 10쪽. 어떤 수리든 **기대값을 먼저 계산**할 수 있다.
  · **잔여 8쪽은 문서당 ±1 이고 둘은 `-1`(우리가 더 적음)** — 한 방향 균일 보정은 존재하지 않는다.
  · **만든 계측기**: `--cells --every-cell`(#292 · 표본 91→12,982) · `--pages`(#294) ·
    블록 판정(#296) · **`BreakReason` 기록**(#299) · **총 쪽 격차 지표**.
  · **내가 여러 번 틀렸고 전부 정정했다**(이슈에 기록):
    ① 형식별 여백 차이를 `lift` 의 `/2` 탓으로 → **layout-check 의 HWPX 는 lift 를 안 탄다**
    ② 「빈 쪽 43개」 → **허위**. `NaiveLayout` 은 표에 `lines` 를 안 만든다 → `place_doc` 으로 교체
    ③ 「들어갔을 것 10%」 → **역산 오류**. 기록으로 보니 **0쪽**, 전부 강제개쪽
    ④ 「이진 지표라 기각이 틀렸을 것」 → 총 격차로 재니 **기각이 옳았다**
    (단 이진 지표가 거친 것은 사실 — 격차 `18→8` 이 일치 `31→32` 로만 보인다)

- 갱신: 2026-09-19 · Claude Opus 5 — **#287 머지 · #245·#253 종결** (main `e71b462`).
  · **원문자(①②③)를 얼굴이 못 그릴 때만 전용 폴백으로 그린다.** 본문 폰트는 안 바꾼다 —
    바꾸면 어드밴스가 움직여 게이트가 흔들린다. 그리기 함수는 `ch: char` 하나를 **고정 좌표**에
    그리므로 그 글자만 얼굴을 바꾸면 **레이아웃이 구조적으로 안 움직인다.**
  · **렌더를 보고 두 번 고쳤다.** ① 무조건 폴백 → macOS 시스템 명조가 **이미 그리던 글리프를
    뺏었다**. ② 커버리지를 **블록당 첫 글자만** 보고 판정 → `㊀㋀`(U+3280~)이 **빈 네모**로
    나왔다(명조가 `㈀`는 갖고 U+3280~은 없다). → **글자 단위** 전수 조회.
    **둘 다 그림을 보고서야 알았다** — `pdffonts` 에 찍히는 것과 보이는 것은 다르다.
  · **폰트**: Noto Sans KR(OFL) 서브셋 445자·99KB. RFN `'Source'` 선언이 있어 OFL 상
    개명(`AutoHwp Enclosed`), `pyftsubset` 이 떨어뜨린 **라이선스 고지를 되살렸다**(OFL 필수).
    `ttf-parser` 는 krilla 가 이미 끌어와 **의존 트리에 새 코드가 늘지 않는다.**
  · **#245 종결**: 원문자 문제는 층이 셋이었다 — HTML(#254) · PDF 텍스트(#254) · **PDF 시각(#287)**.
    제보 문서에서 섹션 머리가 `③ 성장전략(Scale-up)` 으로 나오는 것을 렌더로 확인했다.
    **하나를 고쳤는데 증상이 남으면 「수리가 부족했다」가 아니라 「원인이 하나 더 있다」다.**
  · **#253 종결**: 이슈가 제안한 1번(폰트 교체)은 본문 자신의 경고대로 위험했고, 2번(폴백)이
    `EmbedFont` 다중 슬롯 구조 덕에 슬롯 하나 추가로 끝났다.
  · **찾은 것 → #288**: 명조 얼굴 후보가 번들 OFL `NanumMyeongjo` 를 **아예 안 보고** macOS
    시스템 폰트를 먼저 쓴다. 플랫폼마다 서체가 갈리고 R8 의도와 어긋난다. 렌더를 바꾸므로 분리.
  · `--full` 그린 · **오라클 82건 baseline 동일** · `cargo test -p hwp-export --features pdf` 55/55.

- 갱신: 2026-09-19 · Claude Opus 5 — **#282 머지 · #243 종결** (main `ef780b6`).
  · **원인은 「문단이 자기 글자 크기를 모른다」였다.** `.pN` 은 줄높이만 싣고 크기는 `.cN` 에만
    있어서, 문단 strut 이 문서 값이 아니라 **브라우저 기본 16px** 로 잡혔다.
  · **기준은 「첫 run」이 아니라 「가장 작은 run」이다.** 줄 상자 = `max(문단, span) × 비율` 이라
    **문단이 가장 작은 span 이하면 절대 못 이긴다**. 실측 문단 103개 중 크기가 섞인 것이 10개라
    첫 run 이면 93/103, 최소 run 이면 **103/103**.
  · **인라인인 이유**: `.pN` 은 문단 모양 표의 **공유 클래스**(`p35`=인덱스)라 서로 다른 크기의
    문단이 같은 클래스를 쓴다. 클래스에 박으면 다른 문단이 깨진다.
  · **탈락시킨 D 안**(문단에 자기 문자 클래스를 함께 달기)은 표면이 가장 좁았지만
    **`text-decoration` 이 상속이 아니라 자손에게 전파되고 자식이 끌 수 없어** 밑줄이 문단
    전체로 번진다(해당 클래스 5곳 실재). 이슈 본문에 내가 쓴 A·C 설명 두 군데도 정정했다 —
    A 의 기본값은 최빈(42% 커버)이 아니라 **최소**여야 하고, C 는 이 문서에서 폭발이 아니었다(75개).
  · **#243 종결 실측**(같은 문서, 헤드리스): 표 폭 **636.6px = 원본 636.6px (오차 0.0%)** ·
    18px 행 32 → **20px** · 문서 높이 2010 → **1741px**. 큰 행(42×7 · 48×4)은 원본과 정확히 일치.
    3단계로 나눠 고쳤다 — **#251**(폭을 문서에서) · **#281**(여백) · **#282**(strut).
    **이슈가 짐작한 colgroup/tr height 는 원인이 아니었다** — 이미 구현돼 있었다.
  · **남는 +1~2px 은 의도된 차이**: `10pt×160% = 21.3px` > 원본 18px. 한컴은 셀 높이를 고정하고
    글자를 넣는 것으로 보이는데, 따라가려면 `export-html` 의 **재흘림 계약**을 포기해야 한다.
  · **남긴 사각**: 빈 문단(span 없음)은 여전히 크기를 모른다 — 간격용 빈 줄을 쓰는 양식에서 문제.
  · `--full` 그린 · **오라클 82건 baseline 동일** · `cargo test --workspace` 78 스위트 통과.

- 갱신: 2026-09-18 · Claude Opus 5 — **#281 머지: 셀 세로 여백 5px → HWP 기본값 1.88px** (main `c5afd20`).
  · **`padding: 5px 7px` 은 근거 없는 숫자였다.** 두 군데가 같은 값을 말한다 — 우리 직렬화기가
    레거시 기본값으로 `510/141` 을 문서화(`hwp-jsx/src/lib.rs:720`), 원본 실측도 **셀 67개 전부
    세로 140~141 HWPUNIT**(÷75 = 1.88px). 5px 는 셀 한 변당 +3.1px, **행마다 +6.2px**.
  · **브라우저로 쟀다**(추정 금지 — CSS 계산은 브라우저가 한다). 문서 높이 2010 → **1899px**,
    가장 짧은 행 무리 32px×4 → **26px×4**(원본 18px). 정확히 `2×(5−1.88)=6.24px`.
  · **`--full` 전부 그린** — 게이트 v2 유지 · **오라클 82건 전수 재채점이 baseline 과 동일**.
    HTML 전용 변경이라 조판·PDF 경로를 안 건드린다는 실측 근거다.
  · **범위 밖(의도적)**: 셀별 실제 여백은 `Cell::padding` 이 파서에 있지만
    `hwp-jsx/src/lib.rs:1287` 이 "not yet projected into JSX" 로 못 박아 둔 코덱 계약이라
    손대지 않았다. **#282**(문단 strut 이 브라우저 기본 16px)와 합치면 18px 목표 행이 21px.
  · **CI 함정**: `on: pull_request` 에 `types` 가 없어 **`edited` 가 안 걸린다** — PR 본문에
    `Closes #N` 을 나중에 넣어도 `issue-link` 가 재실행되지 않는다. 커밋을 새로 밀어야 한다.
    연속 머지 때는 보호 규칙이 최신 main 을 요구하므로(`BEHIND`) rebase 후 CI 재통과가 필요하다.

- 갱신: 2026-09-18 · Claude Opus 5 — **#247 제보 문서가 도착했다 — 그리고 `370ec0f` 로는 안 움직였다.**
  · **「제보 문서가 없어」는 이제 사실이 아니다.** 제보자(정부 양식 웹 서비스) 쪽 레포에
    `fixtures/kstartup-reference-pairs/` 로 **`.hwp(x)` + 정부 배포 `.pdf` 쌍 7종**이 모여 있다.
    같은 공고를 한컴으로 조판해 관공서가 배포한 PDF 라 **쪽수 정답지**다. 어떤 문서든 받을 수 있으니
    **다음 세션은 다시 묻지 말 것** — 필요하면 그 경로를 지목하면 된다.
  · **실측 결과([#247 코멘트](https://github.com/kwakseongjae/auto-hwp/issues/247#issuecomment-5727098297))**:
    `370ec0f` 로 다시 쟀는데 **3/6 불일치가 한 자리도 안 바뀌었다** — 모두의창업 2차 39 vs 34(+5) ·
    베트남 테크페스트 25 vs 21(+4) · 재도전 마인드업 4 vs 2(+2). 맞던 3종은 그대로 맞는다.
  · **그래서 #247 에는 원인이 하나 더 있다.** 행높이 수리는 law503(2→1)·`hwp-multi-001`(11→9)을
    실제로 고쳤는데 이 3건은 못 건드렸다. **한 수리가 일부만 고쳤다면 나머지는 다른 원인**이다 —
    쪽수 하나로 묶어 두면 그 사실이 안 보인다(`tac-img-02` 상쇄와 같은 함정).
  · **HTML 행높이(#243 2단계)와는 별개다.** 같은 문서에서 PDF 는 원본 18px 를 정확히 쓰는데
    HTML 만 32px 로 나온다(`td` strut 16px/24px + `padding:5px`). 쪽수는 HTML 을 안 거치는 경로다.

- 갱신: 2026-09-18 · Claude Fable — **#247 재현·수리**: `.hwp` 저장 행높이는 **바닥이 아니라 정확값**이다.
  · **재현**: 제보 문서가 없어 먼저 가진 정답지를 전수 확인 — modu 6==6 · benchmark 8==8 · benchmark1
    18==18 전부 일치. `benchmark2` 의 24 vs 23 은 **정답지가 붙임4 빠진 다른 판본**이라 증거가 아니다
    (#249 오진과 같은 함정). 진짜 재현은 우리 `pdf-calibration-baseline.json` 에 이미 있었다 —
    law.go.kr 공식 쌍 20건 중 **1쪽이 2쪽 되는 2건**. sha256 검증해 받아 재현: 정답지 1 · rhwp 1 ·
    우리 2. **오라클까지 1쪽이라 우리만 틀렸다.**
  · **정량**: 쪽 가용 76,252 / 한컴 표 75,293(여유 959) / 우리 77,767(초과 1,515). **초과 +2,474 가
    55행 중 6행에 집중** — 한컴은 선언 행높이를 지키고 내용이 넘치게 두는데 우리는 행을 키웠다.
  · **원인**: `apply_row_overrides` 의 정확값 경로(#080)를 **HWPX 파서만** 세웠다. `.hwp` 는 교환
    포맷이 아니라 **한컴이 저장한 실제 레이아웃**(lineseg 오라클과 같은 기하)인데 바닥으로 썼다.
  · **테스트가 세 번 잡았다**: ① 자체 HWP5 파서 누락(`hwp5_empty_run_layout`) ② lift 가 HWPX 입력에도
    적용(`hwpx_rhwp_parity` 22 vs 16 → `!from_hwpx` 로 차단) ③ **왕복 손실**(`roundtrip_pages` 18→19)
    — 직렬화기가 `noAdjust="0"` 을 하드코딩해 "이 높이는 정확하다"를 스스로 지우고 내보냈다.
  · **곁다리 개선**: `.hwp`→HWPX 변환이 `noAdjust="1"` 을 싣게 되어 **생산 왕복이 18→18 로 보존**된다
    (이슈 054 갭의 한 조각). rhwp 로 읽으면 19 — rhwp 표 모델에 그 필드가 없다(vendored, 수정 금지).
  · **효과**: law503 2→1 · `hwp-multi-001` 11→9(오라클 일치) · 스윕 쪽격차 11→10 · 게이트 유지.
  · **드러난 것**: `tac-img-02` 68→63(오라클 66). 68 은 *줄 부족(21줄) + 행 과다* 가 상쇄된 값이었다
    — **#274** 로 분리. 쪽수 하나로 품질을 재면 이런 상쇄를 정확도로 오해한다.
...
- 갱신: 2026-09-17 · Claude Fable — **#256 JS 설치 경로 정정 (앞 항목의 진단 오류 수정)**.
  ...
  **틀렸다**. `package-lock.json` 은 **6곳에 커밋돼 있고** CI 도 `npm ci` 를 쓴다(정본은 npm).
  스테일 사본은 레포 결함이 아니라 **내가 `pnpm install` 을 쓴 탓**이다 — pnpm 은 `file:` 의존을
  store 로 **복사**해 설치 시점 dist 가 굳고, npm 은 **심볼릭 링크**라 구조적으로 스테일이 불가능하다.
  실제 결함은 따로 있다: `verify-local.sh --full` 이 fresh clone 에서 설치 단계 없이
  `tsc: command not found` 로 죽고, 스크립트가 `pnpm -C` 를 쓰는 탓에 기여자를 이 오진으로 유도한다.
  수리: JS 단계 앞에 **npm ci 설치 가드** + 빌드/테스트 명령을 **npm 으로 통일**. 브랜치
  `fix/issue-256-js-install-guard`.

- 갱신: 2026-09-17 · Claude Fable — **외부 제보 3건 착지 · 파생 이슈 5건 분리**.
  외부 제품(정부 양식 웹 서비스)의 #243·#244·#245 를 전부 재현했고, **이슈가 적어 온 근본
  원인 후보 3개 중 2개가 틀렸다**. 재현 과정에서 제보에 없던 문제를 3건 더 찾았다.
  | 머지/PR | 내용 | 상태 |
  |---|---|---|
  | `248ad8a` | #244 U+2007 줄바꿈 기회 (조판 1줄 + HTML `<wbr>`) | 머지됨 |
  | `8895bdd` | #246 오라클 전수 재채점 배선 + baseline 판정 | 머지됨 |
  | PR #252 | #251 표 폭을 문서에서 (#243 1단계) | CI 대기 |
  | PR #255 | #254 한컴 심볼 PUA 매핑 (#245 1단계) | CI 대기 |
  · **#244 원인**: 금칙이 아니라 HWP5 고정폭 빈칸 → `U+2007` 에 줄바꿈 기회가 없던 것.
    HWPX(U+3000)는 이미 고쳐져 있던 **반쪽 수정**. HTML 은 조판기를 안 거쳐 별도 지점
    (`overflow-wrap` 제거는 셀 28px 침범 — 오답).
  · **#243 원인**: 이슈가 제안한 colgroup/tr height 는 **이미 구현돼 있었다**. 진짜 결함은
    표 폭이 **뷰포트의 함수**(792/746/652px · 원본 637px). 1단계로 636.6px 고정(오차
    +17%→0.06%), 좁은 화면은 수정 전과 동일 — 플래그 불필요. 2단계(행 높이)는 #243 에 남음.
  · **#245 원인**: `U+F02B1` 은 **평면15 SPUA-A** 라 `subst_glyph` 의 BMP 게이트에 원천적으로
    안 걸림. 매핑표는 rhwp(MIT) 의 시각 검증 확정분을 승격(잠정 2건 제외).
    HTML 완전 복구 · PDF 텍스트 복구 · **PDF 시각은 #253 이 풀려야 함**.
  · **새 이슈**: #249(over-split #206 — sol/Grok 몫) · #251·#254(1단계) ·
    #253(NanumGothic 에 원문자 글리프 없음 — PUA 무관 선행 문제).
  · **계측기 복구**: `verify --full` 이 82건 전수 재채점을 실제로 돈다. #245 검증에서
    처음으로 작동 확인(`ok (82 docs, scorable=79, unscorable=3)`).
  · 환경: `packages/{ai-protocol,editor-core,react}` lockfile 없음 · react 가 보는 editor-core
    dist 사본 스테일(`pnpm install --force` 필요) — 별도 판단 사항.

- 갱신: 2026-09-17 · Claude Fable — **#244 머지(PR #248) · #246 판정 완료 · #249 분리**.
  · **#244 머지됨** (`248ad8a`). U+2007 줄바꿈 기회 — 조판기 1줄 + HTML `<wbr>` 1줄.
  · **#246 판정**: 8-22~9-17 한 달간 82건 중 **3건만** 움직였고, bisect 로 전부 특정했다.
    | 문서 | 이동 | 원인 | 판정 |
    |---|---|---|---|
    | tac-img-02 | 66→67 | `f2306d1` #194 표 캡션 | **실재 캡션 6개**(IR 확인) — 갱신 |
    | tac-img-02 | 67→68 | `2218fb2` #207 인라인 객체 flow | 이미지 6개가 실제 높이 획득 — 갱신 |
    | benchmark1.hwpx | 22→30 | `5903303` #206 over-tall 셀 연속 | **과분할 의심 → #249 분리** |
    | math-001 | 3→2 | (개선) | 줄정확 84.2%→100% — 갱신 |
    tac-img-02 는 **경계 문서 + 오라클 사각지대**다: 저장 lineseg 은 문단 텍스트 레이아웃이라
    캡션·인라인 객체 높이를 못 본다. 우리 줄 수는 한컴보다 **적은데**(본문 1150/1171,
    셀 2960/2970) 쪽수만 많다 — 텍스트 과흐름이 아니라 오라클이 못 세는 높이를
    우리가 세기 시작한 것. 조판 수리 대상 아님.
  · **배선**: `verify-local.sh --full` 에 `oracle-sweep --check` 편입(quick 은 그대로).
    기존엔 `--check-committed`(스키마만)만 돌아 "baseline 이 오늘 코드와 맞는가"를
    영영 검사하지 못했다 — 그래서 한 달을 놓쳤다.
  · baseline 갱신은 **점수 꾸미기가 아니라 판정 기록**이다. #249(과분할)는 갱신으로 덮지 않고
    이슈로 분리해 추적한다 — 계측기를 먼저 살려야 다음 회귀를 볼 수 있기 때문이다.
  · 환경: `packages/{ai-protocol,editor-core,react}` lockfile 없음 · react 가 보는 editor-core
    dist 사본이 스테일이라 `pnpm install --force` 필요했다(별도 판단 사항).

- 갱신: 2026-09-17 · Claude Fable — **외부 제보 #243·#244·#245 재현 · #244 PR #248 · 스윕 #246**.
  외부 제품(정부 양식 웹 서비스)에서 올라온 3건을 전부 재현했고, 이슈의 근본 원인 후보
  3개 중 **2개가 틀렸다**. 셋 다 이슈에 재현 코멘트를 남겼다.
  · **#244(PR #248, 머지 금지)**: 금칙 문제가 아니라 HWP5 고정폭 빈칸 → `U+2007` 에
    줄바꿈 기회가 없던 것. HWPX(U+3000)는 이미 고쳐져 있던 **반쪽 수정**. 조판기 1줄 +
    HTML `<wbr>` 1줄. 게이트 8/18/24 · 스윕 신규 회귀 0 · verify --full 그린.
    HTML은 조판기를 안 거쳐 별도 지점 — `overflow-wrap` 제거는 셀 28px 침범(오답).
  · **#245(진단만)**: `U+F02B1` 은 **평면15 SPUA-A** 라 `subst_glyph`/`old_hangul_cluster`
    의 `0xE000..=0xF8FF` 게이트에 원천적으로 안 걸린다. PDF는 "사라진다"가 아니라
    **일관되지 않는다**(B1 텍스트 없음 / B2~B4 bbox만). 이슈 밖 더 큰 손해: `U+F02EF`
    12개 중 9개 소실 → PDF에서 "제품·서비스"가 "제품 서비스"로 **의미가 바뀐다**.
    고칠 자리는 `subst_glyph`(이미 U+2024/30FB/FF65 → U+00B7 동일 패턴). 다음은 매핑표
    출처·라이선스 근거(`old_hangul.rs` 의 KTUG Public Domain 과 같은 급).
  · **#243(진단만, 결정 대기)**: 이슈가 제안한 colgroup/tr height 는 **이미 구현돼 있다**.
    진짜 결함은 표 폭이 문서가 아니라 **뷰포트의 함수**(1440~840→792px · 794→746px ·
    700→652px · 원본 637px — 어느 뷰포트에서도 안 맞음). 행 높이는 CSS `height` 가
    floor 라서 무시됨 + `td{padding:5px 7px}` 가 원본 `cellMargin`(141HWPUNIT=1.88px)의
    3.7배. 제안: 컨테이너 폭·표 mm·padding 을 문서에서 가져오면 플래그 없이 가능.
    **소유자 확인 대기** — 기본 동작 변경 판단.
  · **#246(신규)**: 82건 오라클 스윕이 main 에서 이미 실패 중.
    bisect 결과 원인 커밋 **`f2306d1`(#194 표 캡션)** — 캡션이 높이를 갖게 되며
    tac-img-02 셀 줄 +2 → 쪽 66→68. 단 줄 정확도는 **개선**(2721→2722)이라
    "#194가 틀렸다"가 아니라 **캡션 높이 과다 vs 경계 문서** 판정이 필요하다.
    근본 문제: `verify-local.sh`·CI 모두 `--check-committed`(스키마만)만 돌고
    **실제 재채점 `--check` 는 아무 데서도 안 돈다**. baseline 은 `9c497d1`(8-22) 이후
    갱신 없음 — 그 뒤 조판 커밋 36개가 한 번도 채점되지 않았다.
  · 환경 메모: `packages/{ai-protocol,editor-core,react}` 에 node_modules 미설치였고,
    설치 후에도 `packages/react` 가 보는 editor-core **dist 사본이 스테일**이라 빌드가
    깨졌다(`pnpm install --force` 로 해소). 세 패키지에 lockfile 이 없다 — 별도 판단 필요.
