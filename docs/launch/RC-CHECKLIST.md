...
| 개인정보 문구 | 소유자 | 완료 | 실제 운영과 `/privacy`를 대조한 뒤 `OWNER-APPROVAL.md`에 승인자/시각 기록 | 승인된 `OWNER-APPROVAL.md` |
| 런칭 문구 | 소유자 | 완료 | CONTENT-BRIEF의 허용/금지 주장과 README 한·영문 승인 | 승인된 `OWNER-APPROVAL.md` |
| durable rate limit | 소유자(저장소 생성)→작업 에이전트(연결 검증) | 베타 유예 | Upstash는 이번 공개 베타에서 소유자 결정으로 유예. 메모리 카운터는 전역 비용 상한이 아님을 공개하고 pending 유지 | `evidence/2026-08-12-beta-rate-limit-deferral.md` |
...
문구를 승인했다. 같은 날 durable Upstash는 이번 공개 베타에서 신경 쓰지 않는 것으로 명시 결정했다.
따라서 해당 게이트는 통과로 가장하지 않고 pending을 유지하되, 아래 위험을 공개한 상태로 RC 배포와
라이브 smoke를 진행한다.

1. 확정할 단일 `release_commit`
2. 같은 SHA의 tag/GitHub Release
3. 배포 뒤 라이브 smoke와 증거 기록

durable rate limit을 닫기 전에는 `stage=ready`로 승격하지 않는다. 이번 배포는 집단지성 참여를 받는
공개 베타이며, 서버리스 인스턴스별 메모리 카운터가 전역 비용 상한을 보장하지 않는 운영 위험을
소유자가 수용했다. 태그·Release·Vercel은 여전히 하나의 SHA에 고정한다.
...
6. 소유자 승인과 Upstash 유예 결정 확인
7. RC SHA 고정→tag/GitHub Release/Vercel을 같은 SHA에서 실행
8. `scripts/launch-readiness.mjs --pre-live --strict` 실행 — durable gate 1건의 예상 실패 외 새 실패 0 확인
9. 종합 라이브 smoke를 실행하고, 성공 후에도 durable gate가 닫힐 때까지 `stage=ready` 승격은 보류
