...
> **경고 — 이 문서의 "부재"·"실측치"는 2026-07-03 시점 스냅샷이다. 대부분 이미 해소됐다.**
> §1(레이턴시 예산표) · §3(인터랙션 격차 카탈로그) · §6(백로그 표)의 "부재" 판정을 현재 상태로 읽지 마라.
> 잔여 격차는 **FG-10(스냅/가이드) · FG-15(다중선택 일괄이동) · FG-16(캔버스 렌더러)** 셋뿐이고,
> FG-01~09 · FG-11~14는 전부 구현 완료다 (FG-01→037, FG-02→034, FG-03/04/05→035, FG-06/09→038,
> FG-07→039, FG-08→036, FG-11→040, FG-12→053, FG-13→059, FG-14→055).
> **현재 상태의 정본은 [`docs/issues/README.md`](./issues/README.md)** — 착수 전 반드시 거기서 대조하라
> (이미 구현된 기능을 다시 기획하는 사고를 막기 위한 배너다).
...
측정 환경: Apple Silicon macOS, headless chromium (playwright 1228), 엔진 = `@auto-hwp/engine`
...
- **메인스레드 블로킹(코드 인용)**: `WasmAdapter.ts:75` `HwpDoc.open(...)` 및 모든 질의(`guard((d)=>fn(d))`,
  47-58행)는 **동기 메인스레드 호출**이다. 워커 없음 — 엔진은 `import { HwpDoc } from "@auto-hwp/engine"` 로
...
/ `nodes.sh`). 게이트 무영향(docs+scripts 추가만): `cargo run -q -p auto-hwp-cli --features "shaper rhwp" -- layout-check
