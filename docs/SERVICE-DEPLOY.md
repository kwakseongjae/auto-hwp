# auto-hwp 헤드리스 서비스 배포 (issue 013)
...
- **`ANTHROPIC_API_KEY`** (R6) — 선택. auto-hwp가 자체 LLM 호출(hwp-ai BYOK)을 하는 경로에서
  ...
  **로그에 남지 않는다**. (MCP 표면 자체는 사전 저작된 content JSON을 받으므로 LLM을 직접
  호출하지 않는다 — 이 키는 CLI `ai-edit`/`ai-fill` 등 auto-hwp 내부 LLM 경로용이다.)
...
docker run -d --name hwp-svc -p 8752:8752 --memory=1g --cpus=1 \
  -e HWP_MCP_TOKEN="$TOKEN" auto-hwp-service
...
auto-hwp의 등가 경로는 `open_document`(.hwp를 rhwp로 lift) → HTML 투영(CLI `export-html`)이다.
...
for f in growth-package-form initial-package-form; do
  cargo run -q -p auto-hwp-cli --features "shaper rhwp" -- export-html "$BP/$f.hwp" -o "/tmp/$f.html"
...
| fixture | test_e2e 임계값 | auto-hwp export-html | 판정 |
|---------|----------------|--------------------|------|
...
즉 auto-hwp 엔진이 pyhwp 기반 서비스와 동등하게 표/셀/병합 구조를 보존한다.

## 8. 바이트 대조 (로컬 CLI vs 컨테이너)

- **HWPX(결정적, 폰트 무관)**: 컨테이너 `open→apply_content→export_hwpx` 결과는 로컬
  `auto-hwp ai-apply <doc> <content.json> -o out.hwpx`(둘 다 `serialize_hwpx`)와 **바이트 동일**해야
  한다. 이것이 패리티의 하드 기준이다.
- **PDF(폰트 discover 의존)**: 컨테이너 `export_pdf`는 로컬 `auto-hwp export-pdf`(둘 다
