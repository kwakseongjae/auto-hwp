...
## 2026-09-22 · 버전은 ㄱㄱ 다음에만 시작

- 각 버전이 끝나면 목표·한 일·화면(스크린샷 또는 영상)을 보고하고, 다음 버전만 브리핑한다.
- 다음 ㄱㄱ의 대상은 #334(0.0.2 도구막대 한 줄). 그 전에는 구현하지 않음.
- 0.1.0 공증은 여전히 별도 승인.

## 2026-09-22 · 0.0.1 실기 진단, 구독 연결을 버전 열차에 넣음

- 로컬 0.0.1로 benchmark.hwp를 염. 8쪽·표는 되고, 편집 크롬이 두 줄, 채팅은 목업.
- Aside 1.0.914 Models에 Claude Subscription이 연결돼 있음. 구독 연결을 0.0.4로 넣기로 바꿈.
- 0.0.2~0.0.9 후 0.1.0은 승인 후에만 공증. 다음 구현은 도구막대 한 줄.
- 알한글은 복구 대화상자를 나중에로 닫음. 사용자 문서 이름은 이슈에 안 올림.
- 다음: 0.0.2 이슈를 열고 그 화면부터.

## 2026-09-22 · 데스크톱 0.1 로드맵 고정 (#331)

- 출시 문장: 채팅이 고치고 페이지에서 확인. 네 동사(문단·표·다음 쪽·PDF).
- 구독 OAuth는 약관/미문서 계약이라 제외. 키체인 API 키.
- 벤치 50은 조판 스윕과 별개. rhwp는 핀 이후 감시만, vendored 수정 없음.
- 문서: `docs/DESKTOP-ROADMAP.md` §0.2. 구현은 아직.
- 다음: 기능 태그 50문서와 네 동사 픽스처. `App.tsx`는 건드리지 않음.

## 2026-09-20 · #247 쪽수 조사 — 계측기 4종, 단일 지렛대 없음 (#292·#294·#296·#299 머지)

- 40건 전수: 일치 31/40 · **총 쪽 격차 18**. 크기(줄·행높이)는 ±0.5% 로 맞는다.
- 여백 축척 · 표 폭 스트레치 **둘 다 고쳐 봤으나 효과 0**. 강제개쪽·keep 도 아님.
- 진짜 성격: break 109개 중 **77개(70%)가 5% 안쪽 아슬**. 예산 = **1% 당 4쪽**.
- 잔여 8쪽은 **문서당 ±1, 양방향** — 균일 보정 불가. 개별 사안.
- 내가 4번 틀렸고 전부 정정(lift 탓·빈 쪽 43개·들어갔을 것 10%·기각 재검토).
- 다음: `-1` 두 건부터(표본 적음) · 문서당 ±1 개별 추적.

## 2026-09-19 · #287 원문자 폴백 · #245·#253 종결 (PR #289 머지)

- 번들 4종에 숫자 원문자가 없다(cmap 실측). 본문 폰트는 안 바꾸고 **못 그리는 글자만** 폴백.
- 렌더를 보고 두 번 고침: 무조건 폴백 → 시스템 명조에서 뺏음 / 블록당 첫 글자 판정 → `㊀㋀` 빈 네모.
- 폰트 = Noto Sans KR(OFL) 서브셋 445자·99KB, RFN 회피 개명 + 라이선스 고지 복원.
- **#245 는 층이 셋**(HTML·PDF텍스트·PDF시각)이었고 마지막이 #253 이었다. 둘 다 닫음.
- 다음: **#288** 명조 후보가 번들 OFL 을 안 본다(플랫폼별 서체 갈림).

## 2026-09-19 · #282 문단 strut · #243 종결 (PR #285 머지)

- `.pN` 이 줄높이만 싣고 크기는 `.cN` 에만 있어 문단 strut 이 브라우저 기본 16px 였다.
  렌더 시 **가장 작은 run** 의 크기를 문단에 인라인으로(첫 run 이면 93/103, 최소면 103/103).
- 실측: 문서 1899→**1741px**, 18px 행 26→**20px**. 문단이 span 보다 큰 경우 **0건**.
- **#243 종결** — 표 폭 오차 **0.0%**(636.6px), 행 높이 +78%→+11%. 3단계(#251·#281·#282).
- 남는 +2px 은 재흘림 계약과의 의도된 차이. 빈 문단은 사각으로 남김.

## 2026-09-18 · #281 셀 세로 여백을 HWP 기본값으로 (#281 · PR #283 머지)
- `padding:5px` → `1.88px`(141 HWPUNIT÷75). 원본 셀 67개 전부 세로 140~141 로 실측 확인.
- 브라우저 실측: 문서 2010→1899px, 32px 행 → 26px. `2×(5−1.88)=6.24px` 와 일치.
- `--full` 그린 · 오라클 82건 baseline 동일 — 조판·PDF 경로 무영향 확인.
- 열린 것: **#282** 문단 strut(브라우저 기본 16px). 셀별 여백은 코덱 계약이라 미착수.

## 2026-09-18 · #247 제보 문서 착지 — 행높이 수리로는 3/6 이 안 움직였다 (#247)
- 제보자 레포에 `.hwp(x)`+정부배포 `.pdf` 쌍 7종(`fixtures/kstartup-reference-pairs/`)이 있다.
  「제보 문서가 없어」우회하던 상태는 끝 — 다음 세션은 다시 묻지 말고 그 경로를 지목할 것.
- `370ec0f` 로 재측정: 3/6 불일치가 **한 자리도 안 바뀜**(+5 · +4 · +2). 맞던 3종은 유지.
- 그래서 #247 에 **원인이 하나 더 있다** — 행높이 수리는 law503·hwp-multi-001 을 실제로 고쳤다.
- 다음: 남은 원인 분리(쪽수 경로는 HTML 을 안 탄다 — #243 2단계와 별개).

## 2026-09-18 · #247 `.hwp` 행높이는 정확값 (#247)
- 재현: law.go.kr T1 쌍(sha256 검증)에서 정답지 1쪽·오라클 1쪽·우리 2쪽. 55행 표에서 +2,474 HWPUNIT
  과다 예약(6행에 집중). 원인 = #080 정확값 경로를 HWPX 파서만 세운 것.
- 수리: rhwp lift(+자체 HWP5 파서)가 `.hwp` 저장 높이를 정확값으로. HWPX 입력은 `!from_hwpx` 로 제외.
  직렬화기가 `noAdjust` 를 실제 값으로 방출 — `.hwp`→HWPX 왕복이 18→18 로 보존된다.
- 효과: law503 2→1 · hwp-multi-001 11→9 · 스윕 쪽격차 11→10 · 게이트/verify 전부 녹색.
- 열린 것: `tac-img-02` 68→63(오라클 66) — 줄 21줄 부족이 드러난 것, #274.

## 2026-09-18 · `packages/platform` (#268)
- 호스트 능력(RPC·이벤트·다이얼로그·테마·드롭·창)을 계약 하나로 묶고 뷰어 UI 3파일을 옮겼다.
  `src/tauri.ts` 밖 `@tauri-apps/*` import 0건을 **테스트가 강제**한다(verify --full 에 배선).
- 검증: 타입체크 · 두 셸 빌드 · 게이트 8/18/24·98.9% · JS 715건(신규 가드 포함) 전부 녹색.
- 열린 것: 이 맥에 미응답 **데스크탑 접근 동의창** 때문에 문서 열기가 멈춘다 — 출시된 0.0.1 도
  동일해서 코드 문제가 아니다. 사용자가 답하면 문서/검수/버전 화면 검수를 마저 한다.
- 알게 된 것: 데스크톱 UI 만 pnpm 정본(`npm install` 은 이전부터 깨져 있었다), pnpm 은 file: 을 복사.

---

## 2026-09-17 (Claude Fable) · 데스크톱 0.0.1 출시 (공증 배포)
- 대시보드·버전 기록(`versions-v1`)·검수·복제. `desktop-v0.0.1` @ `ea5be21`, `/download` 에서 받는다.
- #260 발견·수리: 기본 셸이 OS 파일 열기를 안 받아 **#140 이 배포 빌드에 안 닿아 있었다**.
- 공증 함정 둘: Tauri 는 dmg 를 공증 안 한다 · staple 후 해시가 바뀐다. 둘 다 검증기/페이지에 반영.
- OAuth 는 Anthropic 금지·OpenAI 파트너 한정이라 BYOK+Ollama 로 간다. 다음은 `packages/platform`.

## 2026-09-17 (Claude Fable) · 외부 제보 #243·#244·#245 재현·착지
- 근본 원인 후보 3개 중 **2개가 틀림**. #244=U+2007(HWPX는 이미 고쳐진 반쪽), #243=표 폭이
  뷰포트 함수(제안은 이미 구현됨), #245=평면15라 BMP 게이트 미적용.
- 머지: #244(`248ad8a`) · #246 스윕 배선+baseline 판정(`8895bdd`). PR 대기: #252·#255.
- 파생 3건: #249(over-split #206) · #253(NanumGothic 원문자 글리프 없음) · #243 2단계(행 높이).
- 계측기 복구 — `verify --full` 이 82건 전수 재채점을 실제로 돈다(한 달간 안 돌던 것).
...
## 2026-08-22 (Claude Fable) · sol 인수인계 킷 + 조판 T2 4연타
- 조판: #78·#80·#82 머지(같은 누수 계열). 일치 51→53, 쪽격차 16→10, 하락 0건, 채점 로직 무변경.
- #84(수식·차트 extra 문단)는 진행 중 — 인수 전 PR/워킹트리 상태 확인 필요.
- 인수인계 정본 docs/handoff/CODEX-SOL-2026-08-22.md(#85): 백로그 지도·계측기·프롬프트 4벌·함정 7종.
