...
**1단계 · 양식 업로드** — `.hwp`/`.hwpx`를 올리면(파일 선택 또는 점선 칸에 **끌어다 놓기**) 그 자리에서
라벨(성명·연락처·기업명 …)을 찾아 채울 자리 초안을 만든다. 양식 파일이 없다면 **"샘플로 체험"** 한 번으로
샘플 양식 + 데모 명단 3명이 채워진 상태에서 시작할 수 있다.

**2단계 · 영역 지정 · 규격화** — 문서가 실제로 렌더된 화면에서 셀을 클릭해 채울 자리를 더하거나 뺀다.
...
**3단계 · 명단 붙여넣기** — 아래 네 형식을 자동으로 알아본다. 엑셀에서 표를 복사해 그대로 붙여넣어도 되고,
...
원본 자료(엑셀·메모·기존 문서)를 AI로 정리하고 싶다면 "AI 프롬프트 복사"를 쓸 수 있다. 이 경로만은
**명단의 개인정보가 외부 AI 서비스(ChatGPT 등)로 전송**된다(화면에도 고지된다) — 양식·채움·검증은 그대로
브라우저 안에서만 처리된다.

**4단계 · 검수** — 인원별 결과를 실제로 다시 렌더해서 보여준다. 채운 칸은 문서 위에 하이라이트되고,
◀▶로 한 명씩 넘겨 확인한다. 문제가 있는 행에는 배지가 붙는다. 확인이 끝나면 zip을 내려받는다
(개별 `.hwpx` + `report.json`).
