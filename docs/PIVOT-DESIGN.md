...
```
        ...
        hwp_mcp::Session/Intent/apply_intent (공유 코어, 확장)  ── hwp-render(실구현) ── hwp-typeset
                          │
  ┌──────────┬───────────┼───────────┬──────────────┬───────────┐
auto-hwp-cli  mcp stdio   mcp HTTP   hwp-viewer(Tauri)  Wasm     (전부 동일 코어 호출)
...
**CLI(기존 auto-hwp-cli 확장, 새 바이너리 불필요):**
- `auto-hwp open-project <in.hwp|.hwpx> --out-dir <proj/>` — JSX+CSS 프로젝트 디스크화(요구 2).
- `auto-hwp export-html <in> --mode layout|semantic -o out.html` (D1, D4).
- `auto-hwp export-pdf <in> -o out.pdf` (D1).
- `auto-hwp edit-op <proj/> --intent <intent.json>` — op 1개 적용·재기록(AI 라우팅 CLI 표면, dirty .jsx/.css만 재emit).
- `auto-hwp export-docx <in> -o out.docx` (--features docx).
...
| **auto-hwp-cli** (detect/info/convert/edit/ai-*) | **adapt** | 헤드리스 entry 그대로. export-jsx/import-jsx/export-html/export-pdf/edit-op/export-docx 추가. |
...
- `crates/` 확인 — hwp-render·hwp-typeset·hwp-rhwp·hwp-ops·hwp-ai·hwp-mcp·hwp-core·hwp-hwpx·hwp-viewer·auto-hwp-cli 존재(설계가 가정한 레거시 맵 일치).
