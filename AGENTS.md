...
## 에이전트 역할 분담 (2026-09-18 갱신)
- **Claude = 기획부터 구현·검수까지 직접 맡는다.** 이슈 설계(수용 기준 작성) · 구현 · 테스트 ·
  브라우저 QA · PR · 로드맵 문서 정합 유지. 2026-08-18 표는 실작업을 Grok 4.6 에 맡겼으나
  **Grok/sol 은 더 이상 쓰지 않는다**(사용자 지시, 2026-09-17).
- 공통 규율(issue-first · PR 정지 · 병합은 질문 · UI 브라우저 QA)은 누가 하든 동일하게 적용한다.
  사용자 명시 지시가 이 절보다 우선하며, 다른 에이전트(Codex 등)는 사용자가 지정할 때만 쓴다.
- ⚠️ `.grok/skills/*` 경로는 **문서로서 유효하다**(issue-first · browser-qa 절차의 정본).
  폴더 이름이 grok 일 뿐 특정 모델 전용이 아니다.
